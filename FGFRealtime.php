	...
	public function setActivityCode($activityCode)
	{
		switch ($activityCode)
		{
			...
			case 'E':
				...
				break;
			default:
				$this->activityCode = 'Activity Code: ' . $activityCode; // Activity Code: N OUT
				break;
		}
	}
...
$excel = IOFactory::load('C:\xampp\htdocs\trucknotes\FGF\FGFRealtime.xlsx');
$serviceArray = array();
error_reporting(1);
...
function sendEmail($bin, $pickReturn)
{
	...
	try 
	{
		...
		if (strpos($pickReturn, "BLOCKED") !== false || strpos($pickReturn, "EMPTY") !== false)
		{
			...
			$mail->Body .= 'Please disregard previous email with Workorder ID: "' . $bin->getWorkorderId() . '"; bin is ';
		}
		else
		{
			$mail->Subject = 'TEST - FGF Bin Service';
		}
