...
// for ($t = 1; $t <= 17; $t++) // day of month
// {
	set_time_limit(50000);
	$t = 22; // date
	for ($p = 0; $p <= 23; $p++) // hour
	{
		for ($o = 0; $o <= 55; $o+=5) // minute
		{
			$excel = IOFactory::load('C:\xampp\htdocs\trucknotes\FGF\FGFRealtime-onDemand.xlsx');
			$serviceArray = array();
			error_reporting(1);
			date_default_timezone_set('America/Toronto');
			$vt = sprintf("%08d", $t); // leading zero to day
			$vp = sprintf("%08d", $p); // leading zero to hour
			$vo = sprintf("%08d", $o); // leading zero to minute
			$sDate = new DateTime('2022-05-' . $t);
			$sDate->setTime($p, $o);
			$startDate = $sDate->format('Y-m-d H:i');
			$startDate = str_replace(' ', 'T', $startDate);
			$startDate .= ':00.000Z';

			$eDate = new DateTime('2022-05-' . $t);
			$eDate->setTime($p, $o+5);
			$endDate = $eDate->format('Y-m-d H:i');
...
function save($excel)
{
	...
	$writer->save('./FGFRealtime-onDemand.xlsx');
}
function sendEmail($bin, $pickReturn)
{
	...
	try 
	{
		...
		if (strpos($pickReturn, "BLOCKED") !== false || strpos($pickReturn, "EMPTY") !== false)
		{
			...
			$mail->Body .= 'Please disregard last email with Service Description: "' . $bin->getServiceDescription() . '"; because bin is ' . $pickReturn . '<br>';
		}
		else
		{
